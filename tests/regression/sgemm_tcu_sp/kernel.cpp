...
void kernel_body(kernel_arg_t *__UNIFORM__ arg) {
  ...
  constexpr uint32_t stores_per_col = (NUM_THREADS < PD) ? (PD / NUM_THREADS) : 1;
  constexpr uint32_t meta_cols_per_load = (NUM_THREADS >= PD) ? (NUM_THREADS / PD) : 1;
  constexpr uint32_t num_meta_loads = (meta_cols * stores_per_col + meta_cols_per_load - 1) / meta_cols_per_load;
